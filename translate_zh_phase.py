...
def _build_translation_prompt(items: list[tuple[str, str]]) -> str:
    ...
    lines.append("- Keep game terms: 狼人(Werewolf), 預言家(Seer), 強盜(Robber), 皮匠(Tanner), 爪牙(Minion), 村民(Villager), 搗蛋鬼(Troublemaker)")
    lines.append("- Keep player names in English (The Prosecutor, The Therapist, etc.)")
    lines.append("- Keep role names with their Chinese in parentheses if already present")
    lines.append("")
